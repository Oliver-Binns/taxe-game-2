...
/*
 * This class may have to be re-written due to the dynamic map loading. Station locations and optimal/suboptimal paths are no longer static or guaranteed
 * 
 * 
 * 
 * 
 ...
 * 
 * 
 * Apologies for the inconvenience to whoever has to deal with this mess :/
 * 
 * Your's sincerely,
 * Sam Watkins
 */
...
//        fs = wm.stationList()[1];   //End goal at second station
//
...
//
//        train.route.addConnection(wm.stationList()[0].connections.get(0)); //Connection from stationList()[0] to Dublin
...
//        train = new OilTrain(0, true, new Route(WorldMap.getInstance().stationList()[0]), player);
//        goal.assignTrain(train);
//
//
//        //Go from stationList()[0] to Dublin to Reykjavik to Oslo to Berlin to Dublin
//        train.route.addConnection(wm.stationList()[0].connections.get(0));
